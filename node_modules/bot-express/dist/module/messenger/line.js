...
module.exports = class MessengerLine {
    ...
    async reply(event, messages) {
        ...
        try {
            ...
            if (MessengerLine.is_reply_token_timeout(e) &&
                event &&
                event.source &&
                event.source.userId) {
                // Retry by using send().
    ...
    */
    static async translate_message(translator, message_type, message, sender_language) {
        switch (message_type) {
            case "text": {
                return translator.translate(message.text, sender_language).then((response) => {
            ...
            case "confirm_template": {
                ...
                return translator.translate(source_texts, sender_language).then((response) => {
            ...
            case "carousel_template": {
                ...
                for (let column of message.template.columns) {
                    if (column.title)
                        source_texts.push(column.title);
