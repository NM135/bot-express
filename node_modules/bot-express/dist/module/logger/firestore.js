...
module.exports = class LoggerFirestore {
    ...
    async skill_status(channel_id, user_id, chat_id, skill, status, payload) {
        ...
        if (status === "launched") {
            ...
            if (payload.context)
                skill_status.context = clone(payload.context);
        }
            ...
            if (payload.context)
                skill_status.context = Context.remove_buffer(payload.context);
        }
        else if (status === "switched") {
            // Add next intent and confirming.
            if (payload.intent && payload.intent.name)
                skill_status.intent = payload.intent.name;
            if (payload.context && payload.context.confirming)
                skill_status.confirming = payload.context.confirming;
        }
            ...
            if (payload.context && payload.context.launched_at)
                skill_status.ttc = new Date().getTime() - payload.context.launched_at;
        }
        // Add timestamp.
