...
module.exports = class Flow {
    ...
    is_intent_postback(event) {
        if (this.bot.identify_event_type() !== "postback")
            return false;
        ...
        if (typeof payload !== "object")
            return false;
        if (payload._type !== "intent" && payload.type !== "intent")
            return false;
    ...
    is_process_parameters_postback(event) {
        ...
        if (typeof payload !== "object")
            return false;
        if (payload._type !== "process_parameters" && payload.type !== "process_parameters")
            return false;
    ...
    revive_skill(skill) {
        ...
        this.context.param_change_history.forEach(orig_log => {
            ...
            if (log.param.generator) {
                ...
                if (!generator[log.param.generator.method])
                    throw Error(`${log.param.generator.file} does not have ${log.param.generator.method}`);
    ...
    async process_parameters(parameters) {
        ...
        if (parameters[param.name] === undefined || Array.isArray(parameters[param.name]) && parameters[param.name].length === 0 || parameters[param.name] === "") {
        ...
        try {
            ...
            if (e.name === "Error") {
                ...
                if (typeof param_value === "string" || typeof param_value === "number") {
                    ...
                    if (param_value_str.length > 255)
                        param_value_str = param_value_str.substring(0, 255);
    ...
    async identify_mind(payload) {
        ...
        for (let param_name of all_param_name_list) {
            ...
            parameters_parsed.push(this.bot.parse_parameter(param_name, payload, true).then((response) => {
        ...
        return Promise.all(parameters_parsed).then((responses) => {
            let fit_parameters = [];
    ...
    async change_intent(intent) {
        ...
        // If we find some parameters from initial message, add them to the conversation.
        if (this.context && this.context.intent && this.context.intent.parameters) {
            await this.process_parameters(this.context.intent.parameters);
        }
    }
    /**
